  ...
  try {
    return await new Promise(function (resolve, reject) {
      pool.query(
        ...
        "SELECT CAT.CATEGORY_NAME,CATEGORY_URL,STRING_AGG(DISTINCT C.COURSE_NAME,', ') COURSES,STRING_AGG(DISTINCT C.COURSE_URL,', ') COURSE_URL, COUNT(CO.CID) TOTAL_COLLEGES FROM CATEGORIES CAT JOIN COURSES C ON CAT.CAT_ID=C.CAT_ID LEFT JOIN COLLEGES CO ON CAT.CAT_ID=ANY(STRING_TO_ARRAY(CO.CATEGORIES,',')::INT[]) WHERE CAT.CATEGORY_STATUS='A' AND CO.STATUS='A' GROUP BY CAT.CATEGORY_URL,CAT.CATEGORY_NAME,CAT.GOAL_DISP_POSITION ORDER BY CAT.GOAL_DISP_POSITION ASC LIMIT 15",
        (error, results) => {
          if (error) {
            reject(error);
          }
